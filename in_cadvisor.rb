...
class CadvisorInput < Fluent::Input
  ...
  def emit_container_info(obj)
    ...
    response = RestClient.get(@cadvisorEP + "/subcontainers/system.slice/docker-" + id + ".scope")
    res = JSON.parse(response.body)
    # Loop through containers in pod
    # Set max memory
    res.each do |container|
      memory_limit = @machine['memory_capacity'] < container['spec']['memory']['limit'] ? @machine['memory_capacity'] : container['spec']['memory']['limit']

      latest_timestamp = @dict[id] ||= 0

      # Remove already sent elements
      container['stats'].reject! do | stats |
        Time.parse(stats['timestamp']).to_i <= latest_timestamp
      end

      container['stats'].each_with_index do | stats, index |
        ...
        # out of the array
        if index == (container['stats'].count - 1)
          @dict[id] = timestamp
          break
        end
